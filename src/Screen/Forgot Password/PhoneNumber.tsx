...
import {SendResetOtp} from '../../Services/APIs/auth';
...
const PhoneNumber = () => {
  ...
  const handleSubmiter = async () => {
    ...
    if (error === false) {
      ...
      console.log(phone_no, 'Phone no');
      try {
        setButtonLoader(true);
        let response = await SendResetOtp(phone_no);
        //  setLoading(false);
        if (response.status === 200) {
          setErrormsg('We have sent code to your mobile phone');

          setTimeout(() => {
            navigation.navigate('OtpAuthenticationForgetPassword', {
      ...
      <View style={styles.bodyView}>
        <ScrollView>
          <View style={styles.bodyHeading}>
            <Text style={styles.headingText}>Recover </Text>
